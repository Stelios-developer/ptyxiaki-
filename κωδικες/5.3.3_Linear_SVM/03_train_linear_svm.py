...
RESULTS_DIR = ROOT / "results" / "5.3.3_Linear_SVM"
MODELS_DIR = ROOT / "models" / "5.3.3_Linear_SVM"
RANDOM_STATE = 42
C_VALUES = [0.001, 0.01, 0.1, 1.0, 10.0, 100.0, 1000.0]


def build_model(c_value: float) -> LinearSVC:
    return LinearSVC(
        C=c_value,
        penalty="l2",
        loss="squared_hinge",
...
def train_and_evaluate(name: str) -> dict:
    ...
    joblib.dump(model, MODELS_DIR / f"{name}_linear_svm.joblib")
    return result
...
def main() -> None:
    ...
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    (RESULTS_DIR / "linear_svm_summary.json").write_text(
